...
class ML_KEM:
    ...
    def _decaps_internal(self, dk, c):
        ...
        # dirty fix to identity failed ctxt
    ...
    def encap_key(self, ek):
        m = self.random_bytes(32)
        K, _ = self._G(m + self._H(ek))
        return K, m
    
    def encaps_modded(self, ek, m, hash, dup = 1):
        clist = []
        _, r = self._G(m + self._H(ek))
        for id in range(dup):
            try:
                # Embed the hash of stego here
                c = self._k_pke_encrypt(ek, m, self._H(r+id.to_bytes(1)+hash))
            except ValueError as e:
                raise ValueError(f"Validation of encapsulation key failed: {e = }")
            clist.append(c)
        return clist

    def encaps_hash(self, ek, m, hash):
        _, r = self._G(m + self._H(ek))
        return self._H(r+hash)

    def _decaps_internal_modded_second(self, dk, c, m_prime, r_prime, hash, id):
        ek_pke = dk[384 * self.k : 768 * self.k + 32]
        #z = dk[768 * self.k + 64 :]
        #K_bar = self._J(z + c)
        c_prime = self._k_pke_encrypt(ek_pke, m_prime, self._H(r_prime+id.to_bytes(1)+hash))
        # we need to identity failed ctxt in our application (discard SHAKE garbage)
        #return (b'', select_bytes(K_bar, K_prime, c == c_prime))[c == c_prime]
        return c == c_prime

    def _decaps_internal_modded_first(self, dk, c):
        if len(c) != 32 * (self.du * self.k + self.dv):
            raise ValueError(
                f"ciphertext type check failed. Expected {32 * (self.du * self.k + self.dv)} bytes and obtained {len(c)}"
            )
        ...
        m_prime = self._k_pke_decrypt(dk_pke, c)
        K_prime, r_prime = self._G(m_prime + h)
        return K_prime, m_prime, r_prime

    def decaps_key(self, dk, c):
        try:
            K_prime, _, _ = self._decaps_internal_modded_first(dk, c)
        except ValueError as e:
            raise ValueError(
                f"Validation of decapsulation key or ciphertext failed: {e = }"
            )
    ...
    def decaps_verify(self, dk, c, hash, id):
        K_prime, m_prime, r_prime = self._decaps_internal_modded_first(dk, c)
        return self._decaps_internal_modded_second(dk, c, m_prime, r_prime, hash, id)
    
    def decaps_hash(self, dk, c, hash):
        dk_pke = dk[0 : 384 * self.k]
        h = dk[768 * self.k + 32 : 768 * self.k + 64] # = self._H(ek)
